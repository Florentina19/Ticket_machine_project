...
class TicketMachine(private val origin: String = "ORIGIN") {
    ...
    private val offers = mutableListOf<SpecialOffer>() // optional list
    ...
    fun insertMoney(amount: Double) {
        ...
        println("Inserted: £${String.format("%.2f", insertedAmount)}")
    }

    private fun bestOfferForStation(station: Station): SpecialOffer? {
        val today = LocalDate.now()
        return offers
            .filter { it.stationName.equals(station.name, ignoreCase = true) && it.isActive(today) }
            .minByOrNull { it.discountFactor } // if multiple, choose best (lowest factor)
    }
    ...
    fun buyTicket(destinationName: String, type: TicketType): Boolean {
        ...
        if (insertedAmount < price) {
            println("Insufficient funds. Price: £${String.format("%.2f", price)}, inserted: £${String.format("%.2f", insertedAmount)}")
            return false
        }
        ...
        // update machine & station
        insertedAmount -= price
        station.increaseSales(1)
        totalTakings += price

        println("Purchase successful. Remaining credit: £${String.format("%.2f", insertedAmount)}")
        return true
    }
    ...
    fun refund(): Double {
        ...
        insertedAmount = 0.0
        println("Refunded £${String.format("%.2f", refundAmount)}")
        return refundAmount
    }
    ...
    fun viewAllStations() {
        ...
        println("Stations:")
        stations.forEach { s ->
            println("${s.name} | Single: £${String.format("%.2f", s.singlePrice)} | Return: £${String.format("%.2f", s.returnPrice)} | Sales: ${s.salesCount}")
        }
    }
    ...
    fun addStationInteractive() {
        ...
        val single = readLine()?.toDoubleOrNull() ?: run { println("Invalid price"); return }
        print("Enter return price: ")
        val ret = readLine()?.toDoubleOrNull() ?: run { println("Invalid price"); return }
    ...
    fun deleteOffersForStation(stationName: String) {
        val removed = offers.removeIf { it.stationName.equals(stationName, ignoreCase = true) }
        if (removed) println("Offers removed for $stationName") else println("No offers found for $stationName")
    }
